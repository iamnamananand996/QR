...
class PhotoBoothApp:
    def __init__(self, outputPath):
        ...
        lbl = tki.Label(self.root, 
            text="Enter the IP address written on the app\n(e.g. 192.168.1.155:8080)")
        lbl.grid(row=0, column=1, padx=10, pady=20)
        self.entry = tki.Entry(self.root)
        self.entry.grid(row=1, column=1)
        btn_0 = tki.Button(self.root, text="Enter", command=self.IP)
        btn_0.grid(row=1, column=2, padx=10, pady=20)
    ...
    def IP(self):
        ...
        if self.entry.get() != '':
            self.vs = cv2.VideoCapture('http://{}/video'.format(self.entry.get()))
            self.elements()

    def elements(self):
        btn_1 = tki.Button(self.root, text="Sticker Image",
                           command=self.takeSnapshot)
        btn_1.grid(row=2, column=1, padx=10, pady=30)

        btn_2 = tki.Button(self.root, text="Hand Written Image",
                           command=self.takeSnapshot)
        btn_2.grid(row=2, column=2, padx=10, pady=20)
...
#vs = cv2.VideoCapture('http://25.66.227.153:8080/video')
    #192.168.1.155:8080/video')
pba = PhotoBoothApp("output")
pba.root.mainloop()
